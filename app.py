...
if st.session_state["authentication_status"]:
    ...
    dash, conciliacao, contapagar, contareceber, container, loft1, loft2, equalizar = st.tabs(['Dashboard', 'Extrato', 'Contas a Pagar', 'Contas a Receber', 'Hotel Container', 'Loft 1', 'Loft 2', 'Equalização'])
    ...
    with contareceber:
        ...
        with col1:
            totalcontasreceber_pendente = f"R$ {planilhacontasreceber_pendente['Total'].sum():,.2f}".replace(",", "v").replace(".", ",").replace("v", ".")
            st.metric("TOTAL EM CONTRATOS À RECEBER:", totalcontasreceber_pendente)

            totalcontasreceber_pago = f"R$ {planilhacontasreceber_pago['Total'].sum():,.2f}".replace(",", "v").replace(".", ",").replace("v", ".")
            st.metric("TOTAL RECEBIDO:", totalcontasreceber_pago)
        ...
        st.subheader("Total em Contratos")
        st.dataframe(planilhacontasreceber_pendente, column_order=["Data", "Vencimento", "Descrição", "Fornecedor", "Valor", "Parcela", "Plano-Contas", "Status"])
        st.subheader("Total Recebido")
        st.dataframe(planilhacontasreceber_pago, column_order=["Pagamento", "Descrição", "Fornecedor", "Valor", "Parcela", "Categoria", "Plano-Contas", "Status"])
    with conciliacao:
        st.subheader("EXTRATO")
        st.dataframe(extrato) 
       
        # extrato['Total'] = pd.to_numeric(extrato['Total'].str.replace('.', '').str.replace(',', '.'), errors='coerce')

        total_extrato = f"R$ {extrato['Total'].sum():,.2f}".replace(",", "v").replace(".", ",").replace("v", ".")
        st.metric("SALDO:", total_extrato)
